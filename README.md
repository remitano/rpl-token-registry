...
@remitano/rpl-token-registry

[![npm](https://img.shields.io/npm/v/@remitano/rpl-token-registry)](https://unpkg.com/@remitano/rpl-token-registry@latest/) [![GitHub license](https://img.shields.io/badge/license-APACHE-blue.svg)](https://github.com/remitano/rpl-token-registry/blob/b3fa86b3fdd9c817139e38641d46c5a892542a52/LICENSE)
...
```bash
npm install @remitano/rpl-token-registry
```

```bash
yarn add @remitano/rpl-token-registry
```
...
import { TokenListProvider, TokenInfo } from '@remitano/rpl-token-registry';
...
* `logoURI` 
  ...
  * the logo can be hosted in this repo in `assets/mainnet/TOKEN_ADDRESS/FILE` 
    * in that case, the image should be added to this repo and logoURI should point to `https://raw.githubusercontent.com/remitano/rpl-token-registry/master/assets/mainnet/TOKEN_ADDRESS/FILE`)
...
```
git remote add pub-origin git@github.com:remitano/rpl-token-registry.git
...
Remitano network explorer, and wallets all pull from this repo at different cadences.  Some update every few days.  
...
are done with the Renec Foundation's ("RF") good faith efforts. It is up to
